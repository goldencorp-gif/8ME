...
const PropertyDetailView: React.FC<PropertyDetailViewProps> = ({ 
       ...
       reader.onloadend = () => {
          const updatedDocs = (property.documents || []).map(d => {
             if (d.id === replacingDocId) {
                // Explicitly cast to PropertyDocument to avoid TS type mismatch on 'type' string vs literal
                return {
                   ...d,
                   name: file.name,
  ...
  const generateInvoiceHtml = (doc: PropertyDocument) => {
    ...
    // 1. Logo Size Increase (120px)
    const logoHtml = doc.content.logoUrl 
      ? `<img src="${doc.content.logoUrl}" style="height: 120px; margin-right: 10px; vertical-align: middle; object-fit: contain;" />` 
      : '';

    // 3. Smart Signature Detection
    const contentString = JSON.stringify(doc.content).toLowerCase();
    const requiresSignature = contentString.includes('sign') || contentString.includes('signature');
    
    const signatureHtml = requiresSignature ? `
      <div style="margin-top: 80px; page-break-inside: avoid;">
        <div style="display: flex; gap: 40px;">
            <div style="flex: 1;">
                <div style="border-bottom: 1px solid #333; height: 40px;"></div>
                <p style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #9ca3af; margin-top: 8px;">Authorized Signature</p>
            </div>
            <div style="flex: 1;">
                <div style="border-bottom: 1px solid #333; height: 40px;"></div>
                <p style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #9ca3af; margin-top: 8px;">Date</p>
            </div>
        </div>
      </div>
    ` : '';
    ...
    return `
      ...
      <head>
        ...
        <style>
          ...
          .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 50px; border-bottom: 2px solid #f3f4f6; padding-bottom: 20px; }
          .logo { font-weight: 900; font-size: 24px; color: #4F46E5; display: flex; align-items: center; gap: 20px; }
  ...
  const handlePrintPdf = (doc: PropertyDocument, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    
    if (doc.content) {
        // Open the generated HTML in a new window and trigger print
        ...
        alert("This file cannot be converted to PDF in the demo environment.");
    }
  };

  const handleExportCsv = (doc: PropertyDocument, e?: React.MouseEvent) => {
      if (e) e.stopPropagation();
      if (doc.content && doc.content.items) {
          const items = doc.content.items as any[];
          const csvContent = "data:text/csv;charset=utf-8," 
              + "Description,Amount\n"
              + items.map(i => `"${i.description}",${i.amount}`).join("\n")
              + `\nTotal,${doc.content.totalAmount}`;
          
          const encodedUri = encodeURI(csvContent);
          const link = document.createElement("a");
          link.setAttribute("href", encodedUri);
          link.setAttribute("download", `${doc.name.replace('.pdf', '')}_data.csv`);
    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 max-w-3xl w-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 rounded-l-[3rem] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-10 bg-slate-50/20">
          ...
          {/* ... [Other tabs omitted for brevity - no changes there] ... */}
          {activeTab === 'owner' && (
            <div className="space-y-8 animate-in fade-in duration-300">
               {/* Owner Profile Card */}
            ...
            <div className="space-y-8 animate-in slide-in-from-bottom-6 duration-500">
              {!selectedFolder ? (
                <div className="grid grid-cols-2 gap-8">
                  ...
                  {renderFolderItem('Inspection', <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 00-2-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>, 'Condition Reports')}
          ...
          <div className="relative w-full max-w-3xl bg-white rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
               ...
               <div className="flex space-x-2">
                 ...
                 {previewDoc.content?.items && (
                    <button 
                      onClick={(e) => handleExportCsv(previewDoc, e)}
                      className="p-2 hover:bg-emerald-50 hover:text-emerald-600 rounded-full transition-colors flex items-center gap-2 px-3 group"
                      title="Export Data to Excel/CSV"
                    >
                      <span className="text-[10px] font-bold uppercase tracking-widest hidden group-hover:block">Export CSV</span>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                    </button>
                 )}
                 {/* Print / PDF Button */}
                 <button 
                   onClick={(e) => handlePrintPdf(previewDoc, e)}
                 ...
                 >
                   <span className="text-[10px] font-bold uppercase tracking-widest hidden group-hover:block">Print / PDF</span>
                   <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                 </button>
              ...
              {previewDoc.content ? (
                previewDoc.content.customHtml ? (
                  ...
                  <div className="bg-white p-12 shadow-xl max-w-2xl w-full min-h-[600px] flex flex-col relative overflow-hidden">
                     ...
                     <div className="flex justify-between items-start mb-12">
                        <div className="flex items-center space-x-3">
                           {previewDoc.content.logoUrl ? (
                             <img src={previewDoc.content.logoUrl} alt="Logo" className="h-24 w-auto object-contain" />
                           ) : (
                     ...
                     {(JSON.stringify(previewDoc.content).toLowerCase().includes('sign') || JSON.stringify(previewDoc.content).toLowerCase().includes('signature')) && (
                        <div className="mt-16 flex gap-10 page-break-inside-avoid">
                            <div className="flex-1">
