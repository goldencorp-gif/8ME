...
const PropertyDetailView: React.FC<PropertyDetailViewProps> = ({ 
       ...
       reader.onloadend = () => {
          const updatedDocs = (property.documents || []).map(d => {
             if (d.id === replacingDocId) {
                // Return a complete object cast as PropertyDocument to satisfy strict TS
                return {
                   ...d,
                   name: file.name,
  ...
  // ... [Rest of file content remains unchanged, keeping all existing render logic] ...
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="absolute inset-y-0 right-0 max-w-3xl w-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 rounded-l-[3rem] overflow-hidden">
        ...
        {/* Keeping existing Tabs content implementation largely same but omitting for brevity in this response as focus is on the function above */}
        <div className="flex-1 overflow-y-auto p-10 bg-slate-50/20">
          ...
          {/* ... [Keeping existing Owner, Vault, Comms tabs exactly as they were] ... */}
          {activeTab === 'owner' && (
            <div className="space-y-8 animate-in fade-in duration-300">
               {/* Owner Profile Card */}
      ...
      {previewDoc && (
        // ... [No changes needed in preview modal logic]
